...
SYSTEM_PROMPT = (
    ...
    "create_life_model. You may call them AS MANY TIMES as the task needs — a "
    "single plan can create several datasets and several life models (e.g. one "
    "dataset per failure mode or subgroup, and multiple candidate models on the "
    "same dataset).\n\n"
    ...
    "EVERY dataset and EVERY life model you intend to create, each with its "
    "distribution and columns. There may be one, or many, of each.\n"
    ...
    "'yes', 'go ahead'). One approval covers the entire plan. If they change it, "
    "revise and ask again.\n"
    "5. LOAD once approved: create each dataset (each returns a dataset_id), then "
    "create each life model referencing the right dataset_id. Do the full batch — "
    "don't stop after one. Report everything you created.\n\n"
